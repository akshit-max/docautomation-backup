    ...
    <div style={f.wrap}>
      ...
      {(content.line_items || []).map((item: any, i: number) => (
        <div key={i} style={f.card}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6, marginTop: 6 }}>
            ...
            {/* Amount is auto-calculated from hours × unit_price */}
            <div style={f.group}>
      ...
      {gstEnabled && (
        <>
          ...
          <Field
            label={content.gst_type === "amount" ? "GST (₹)" : "GST (%)"}
...
export function InvoiceFields({ content, update }: { content: any, update: (k: string, v: any) => void }) {
  ...
  useEffect(() => {
    ...
    // Calculation order: line items → subtotal → discount → taxable → GST → total
    ...
    update("formatted_discount", discountAmt > 0 ? `-₹${discountAmt.toLocaleString("en-IN")}` : "");
    update("taxable_amount", (discountAmt > 0 || gstAmt > 0) ? fmt(taxable) : "");
  ...
  return (
    <div style={f.wrap}>
      ...
      <Field
        label={content.discount_type === "percent" ? "Discount (%)" : "Discount (₹)"}
